def unique_houses(filename):
    ...
    """
    #telling python to iterate over file 
    cohort_data = open(filename)
    #assigning "houses" an empty set to put each house in
    houses = set()
    #"for loop" to iterate over the file and add house name to the houses set
    for line in cohort_data:
        #method that is operating on the line to split file
        data = line.split("|")
        #indexing data for variable house
        house = data[2]
        #if statement to remove any empty items at house index location
        if house == "":
            continue
        else: 
            #adds house names to houses set
            houses.add(house)
